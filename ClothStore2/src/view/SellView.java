...
public class SellView extends JFrame {
   ...
   // 변수들
   JTextField tfPCode, tfPSell, tfPStock, tfPTotal,tfPTell;
   ...
   int i = 0;
   // 장바구니 내용 저장
   Vector<String> sellPCode = new Vector<String>();
   Vector<String> sellPName = new Vector<String>();
   Vector sellStock = new Vector();
   Vector sellPPrice = new Vector();
   ...
   public void addSell() {
      ...
      sellPName.add(name);
      sellStock.add(stock);
      sellPPrice.add(price * Integer.parseInt(stock));

      System.out.println(sellPCode);//상품코드
      System.out.println(sellPName);//상품명
      System.out.println(sellPPrice);//상품가격
      System.out.println(sellStock);//상품갯수

      ShopBasket_name.setListData(sellPName); // 리스트에 값 업데이트
      ShopBasket_price.setListData(sellPPrice);
      ShopBasket_stock.setListData(sellStock);

      total += (int) sellPPrice.get(i);
      i++;
   ...
   public void buySell() {
      ...
      String tell = tfPTell.getText();
      // 장바구니안의 내용을 삽입시킴
      try {
         model.insertSell(sellPCode, sellPPrice, sellStock,tell);
      } catch (Exception e) {
         System.out.println("구매실패:" + e.getMessage());
         e.printStackTrace();
      }
      i=0;
      // 장바구니 초기화
   ...
   public void deleteSell() {
      ...
      total -= (int) sellPPrice.get(temp);

      sellPCode.remove(temp);
      sellPName.remove(temp);
      sellPPrice.remove(temp);
      sellStock.remove(temp);
      System.out.println(temp);
      System.out.println(sellPCode);
      System.out.println(sellPName);
      System.out.println(sellPPrice);
   ...
   public void addLayout() {
      ...
      Right_up_down.setLayout(new GridLayout(5, 2));
      ...
      Right_up_down.add(new JLabel("전화번호"));
      Right_up_down.add(tfPTell);
